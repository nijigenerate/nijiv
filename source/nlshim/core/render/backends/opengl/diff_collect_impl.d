module nlshim.core.render.backends.opengl.diff_collect_impl;

import nlshim.core.diff_collect;
...
package(nlshim) void oglSetDifferenceAggregationEnabled(bool enabled) {
    gDifferenceEvaluator.setEnabled(enabled);
}

package(nlshim) bool oglIsDifferenceAggregationEnabled() {
    return gDifferenceEvaluator.isEnabled();
}

package(nlshim) void oglSetDifferenceAggregationRegion(DifferenceEvaluationRegion region) {
    gDifferenceEvaluator.setRegion(region);
}

package(nlshim) DifferenceEvaluationRegion oglGetDifferenceAggregationRegion() {
    return gDifferenceEvaluator.getRegion();
}

package(nlshim) bool oglEvaluateDifferenceAggregation(RenderResourceHandle texture, int viewportWidth, int viewportHeight) {
    return gDifferenceEvaluator.evaluate(cast(GLuint)texture, viewportWidth, viewportHeight);
}

package(nlshim) bool oglFetchDifferenceAggregationResult(out DifferenceEvaluationResult result) {
...
package(nlshim) void oglSetDifferenceAggregationEnabled(bool) {}
package(nlshim) bool oglIsDifferenceAggregationEnabled() { return false; }
package(nlshim) void oglSetDifferenceAggregationRegion(DifferenceEvaluationRegion) {}
package(nlshim) DifferenceEvaluationRegion oglGetDifferenceAggregationRegion() {
    return DifferenceEvaluationRegion.init;
}
package(nlshim) bool oglEvaluateDifferenceAggregation(RenderResourceHandle, int, int) { return false; }
package(nlshim) bool oglFetchDifferenceAggregationResult(out DifferenceEvaluationResult result) {
    result = DifferenceEvaluationResult.init;
    return false;
}
